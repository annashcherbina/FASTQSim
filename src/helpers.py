...
def insertSpikedReads(qualSource,insertedList,qualname,qualSourceDict):
    ...
    for readlength in insertedList.keys(): 
        if readlength==0:
            continue;
        if readlength in qualSourceDict:     
            closestLength=readlength
        else:
            closestLength=takeClosest(sortedQualKeys,readlength)
        numoptions=len(qualSourceDict[closestLength])-1
        if numoptions==0:
            continue;
        ...
        for i in range(len(insertedList[readlength])): 
            ...
            if len(read) > len(scores): 
                read=read[0:len(scores)] 
            elif len(read) < len(scores): 
                #pad the read
                read=padRead(read,len(scores))
